...
class Embedding_model(nn.Module):
    ...
    def __init__(self,model,input_feat_size=512,embed_dim=2,loss='softmax',
        ...
        self.weight = weight
        self.input_feat_size = model.num_features
        ...
        if (embed_dim == input_feat_size) or (embed_dim is None):
            ...
            self.encoder = nn.Sequential(
                nn.Linear(input_feat_size,self.embed_dim),
        ...
        if loss == 'softmax':
            ...
            if device == 'cuda':
                ...
                use_gpu = False
            self.reg_method = CenterLoss(num_classes=model.num_classes, 
                                         feat_dim=self.embed_dim,use_gpu=use_gpu)
            
        #Learn ACE parameters (Random initalization) with single background stat
        elif loss == 'LACE': 
            self.reg_method = ACELoss(num_classes=model.num_classes,
                                      feat_dim=self.embed_dim,learn=True,
                                      single_stats=True,weight=self.weight,
                                      device=self.device)
            
        elif loss == 'AMC': 
            self.reg_method = AMCLoss(self.embed_dim, model.num_classes,device=device)
            
        elif loss == 'LACES_mean': 
            self.reg_method = ACELoss_Ablation(num_classes=model.num_classes,
            ...
            self.reg_method = AngularPenaltySMLoss(self.embed_dim, model.num_classes, 
        ...
        if loss == 'softmax':
            self.fc = nn.Linear(self.embed_dim,model.num_classes)
        else:
