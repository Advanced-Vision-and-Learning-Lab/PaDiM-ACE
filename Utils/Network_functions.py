...
from Utils.learning_curve import learning_curve
import pdb
...
def train_model(model, dataloaders, criterion, optimizer, device,
    ...
    try:
            ...
            for phase in ['train', 'val']:
                ...
                if phase == 'train':
                    if scheduler is not None:
                        scheduler.step()
                    train_error_history.append(epoch_loss)              # values to plot for loss learning curve
                    train_error_class_history.append(epoch_loss_class)
                    train_error_embed_history.append(epoch_loss_embed)
                    train_acc_history.append(epoch_acc)                 # values to plot for acc learning curve
                ...
                if phase == 'val':
                    ...
                    val_error_history.append(epoch_loss)                # values to plot for learning curve (val loss)
                    val_error_class_history.append(epoch_loss_class)
                    val_error_embed_history.append(epoch_loss_embed)
                    val_acc_history.append(epoch_acc)                   # values to plot for learning curve (val acc)
        ...
        # learning_curve(num_epochs,train_error_history, train_acc_history, val_error_history, val_acc_history, loss_type)
