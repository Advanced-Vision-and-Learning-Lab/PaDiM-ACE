...
class MSTAR_Dataset(torch.utils.data.Dataset):
    ...
    def _load_data(self, path):
        ...
        # the dataloader issue is here, the dataset does not contain any files with a .npy extensions so it doesn't load anything
        image_list = glob.glob(os.path.join(path, f'{self.name}/{mode}/*/*.npy'))
        print((os.path.join(path, f'{self.name}/{mode}/*/*.npy')))
